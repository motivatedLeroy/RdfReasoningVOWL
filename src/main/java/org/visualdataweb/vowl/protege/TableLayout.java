...
import org.visualdataweb.vowl.protege.TableLayoutConstants;
import org.visualdataweb.vowl.protege.TableLayoutConstraints;
...
public class TableLayout implements LayoutManager2, Serializable, TableLayoutConstants {
    ...
    public TableLayout(double[][] size) {
        ...
        if (size != null && size.length == 2) {
            ...
            for(int counter = 0; counter < this.columnSpec.length; ++counter) {
                if (this.columnSpec[counter] < 0.0D && this.columnSpec[counter] != -1.0D && this.columnSpec[counter] != -2.0D && this.columnSpec[counter] != -3.0D) {
                    this.columnSpec[counter] = 0.0D;
                }
            }

            for(int counter = 0; counter < this.rowSpec.length; ++counter) {
                if (this.rowSpec[counter] < 0.0D && this.rowSpec[counter] != -1.0D && this.rowSpec[counter] != -2.0D && this.rowSpec[counter] != -3.0D) {
                    this.rowSpec[counter] = 0.0D;
                }
            }
        } else {
            tempCol = new double[]{-1.0D};
            tempRow = new double[]{-1.0D};
    ...
    public TableLayoutConstraints getConstraints(Component component) {
        ...
        while(iterator.hasNext()) {
            org.visualdataweb.vowl.protege.TableLayout.Entry entry = (org.visualdataweb.vowl.protege.TableLayout.Entry)iterator.next();
    ...
    public void setConstraints(Component component, TableLayoutConstraints constraint) {
        if (component == null) {
            ...
            while(iterator.hasNext()) {
                org.visualdataweb.vowl.protege.TableLayout.Entry entry = (org.visualdataweb.vowl.protege.TableLayout.Entry)iterator.next();
                if (entry.component == component) {
                    iterator.set(new org.visualdataweb.vowl.protege.TableLayout.Entry(component, constraint));
                }
            }
    ...
    public void drawGrid(Container container, Graphics g) {
        ...
        for(int row = 0; row < this.rowSize.length; ++row) {
            ...
            for(int column = 0; column < this.columnSize.length; ++column) {
                Color color = new Color((int)(Math.random() * 1.6777215E7D));
                g.setColor(color);
    ...
    public boolean overlapping() {
        ...
        if (numEntry == 0) {
            ...
            org.visualdataweb.vowl.protege.TableLayout.Entry[] entry = (org.visualdataweb.vowl.protege.TableLayout.Entry[])this.list.toArray(new org.visualdataweb.vowl.protege.TableLayout.Entry[numEntry]);
    ...
    protected void calculateSize(Container container) {
        ...
        for(counter = 0; counter < numColumn; ++counter) {
            if (this.columnSpec[counter] >= 1.0D || this.columnSpec[counter] == 0.0D) {
                this.columnSize[counter] = (int)(this.columnSpec[counter] + 0.5D);
                availableWidth -= this.columnSize[counter];
            }
        }

        for(counter = 0; counter < numRow; ++counter) {
            if (this.rowSpec[counter] >= 1.0D || this.rowSpec[counter] == 0.0D) {
                this.rowSize[counter] = (int)(this.rowSpec[counter] + 0.5D);
        ...
        org.visualdataweb.vowl.protege.TableLayout.Entry entry;
        Dimension p;
        int slackWidth;
        for(counter = 0; counter < numColumn; ++counter) {
            if (this.columnSpec[counter] == -2.0D || this.columnSpec[counter] == -3.0D) {
                relativeWidth = 0;
                iterator = this.list.listIterator(0);

                while(iterator.hasNext()) {
                    entry = (org.visualdataweb.vowl.protege.TableLayout.Entry)iterator.next();
                    if (entry.col1 == counter && entry.col2 == counter) {
                        p = this.columnSpec[counter] == -2.0D ? entry.component.getPreferredSize() : entry.component.getMinimumSize();
        ...
        for(counter = 0; counter < numRow; ++counter) {
            if (this.rowSpec[counter] == -2.0D || this.rowSpec[counter] == -3.0D) {
                relativeWidth = 0;
                iterator = this.list.listIterator(0);
                ...
                while(iterator.hasNext()) {
                    ...
                    if (entry.row1 == counter && entry.row2 == counter) {
                        p = this.rowSpec[counter] == -2.0D ? entry.component.getPreferredSize() : entry.component.getMinimumSize();
        ...
        for(counter = 0; counter < numColumn; ++counter) {
            if (this.columnSpec[counter] > 0.0D && this.columnSpec[counter] < 1.0D) {
                this.columnSize[counter] = (int)(this.columnSpec[counter] * (double)relativeWidth + 0.5D);
                availableWidth -= this.columnSize[counter];
            }
        }

        for(counter = 0; counter < numRow; ++counter) {
            if (this.rowSpec[counter] > 0.0D && this.rowSpec[counter] < 1.0D) {
                this.rowSize[counter] = (int)(this.rowSpec[counter] * (double)relativeHeight + 0.5D);
                availableHeight -= this.rowSize[counter];
            }
        }
        ...
        for(counter = 0; counter < numColumn; ++counter) {
            if (this.columnSpec[counter] == -1.0D) {
                ++numFillWidth;
            }
        }

        for(counter = 0; counter < numRow; ++counter) {
            if (this.rowSpec[counter] == -1.0D) {
                ++numFillHeight;
            }
        }
        ...
        for(counter = numColumn - 1; counter >= 0; --counter) {
            if (this.columnSpec[counter] == -1.0D) {
                this.columnSize[counter] += slackWidth;
                break;
            }
        }
        ...
        for(counter = numRow - 1; counter >= 0; --counter) {
            if (this.rowSpec[counter] == -1.0D) {
                this.rowSize[counter] += slackHeight;
                break;
            }
        }
    ...
    public Dimension preferredLayoutSize(Container container) {
        int scaledWidth = 0;
        int scaledHeight = 0;
        double fillWidthRatio = 1.0D;
        double fillHeightRatio = 1.0D;
        int numFillWidth = 0;
        int numFillHeight = 0;
        ...
        for(counter = 0; counter < this.columnSpec.length; ++counter) {
            ...
            } else if (this.columnSpec[counter] == -1.0D) {
                ++numFillWidth;
            }
        }
        ...
        for(counter = 0; counter < this.rowSpec.length; ++counter) {
            ...
            } else if (this.rowSpec[counter] == -1.0D) {
                ++numFillHeight;
            }
        }
        ...
        if (fillWidthRatio < 0.0D) {
            fillWidthRatio = 0.0D;
        }

        if (fillHeightRatio < 0.0D) {
            fillHeightRatio = 0.0D;
        }
        ...
        for(counter = 0; counter < this.columnSpec.length; ++counter) {
            if (this.columnSpec[counter] == -2.0D || this.columnSpec[counter] == -3.0D) {
                ...
                while(iterator.hasNext()) {
                    entry = (org.visualdataweb.vowl.protege.TableLayout.Entry)iterator.next();
                    if (entry.col1 == counter && entry.col2 == counter) {
                        Dimension p = this.columnSpec[counter] == -2.0D ? entry.component.getPreferredSize() : entry.component.getMinimumSize();
        ...
        for(counter = 0; counter < this.rowSpec.length; ++counter) {
            if (this.rowSpec[counter] == -2.0D || this.rowSpec[counter] == -3.0D) {
                int maxHeight = 0;
                ListIterator iterator1 = this.list.listIterator(0);

                while(iterator1.hasNext()) {
                    entry = (org.visualdataweb.vowl.protege.TableLayout.Entry)iterator1.next();
                    if (entry.row1 == counter && entry.row1 == counter) {
                        Dimension p = this.rowSpec[counter] == -2.0D ? entry.component.getPreferredSize() : entry.component.getMinimumSize();
        ...
        while(true) {
                            ...
                            do {
                                do {
                                    if (!iterator.hasNext()) {
                                        ...
                                        for(counter = 0; counter < this.columnSpec.length; ++counter) {
                                            if (this.columnSpec[counter] >= 1.0D) {
                                                totalWidth += (int)(this.columnSpec[counter] + 0.5D);
                                            } else if (this.columnSpec[counter] == -2.0D || this.columnSpec[counter] == -3.0D) {
                                        ...
                                        for(counter = 0; counter < this.rowSpec.length; ++counter) {
                                            if (this.rowSpec[counter] >= 1.0D) {
                                                scalableWidth += (int)(this.rowSpec[counter] + 0.5D);
                                            } else if (this.rowSpec[counter] == -2.0D || this.rowSpec[counter] == -3.0D) {
            ...
            double relativeWidth = 0.0D;

            for(counter = entry.col1; counter <= entry.col2; ++counter) {
                if (this.columnSpec[counter] > 0.0D && this.columnSpec[counter] < 1.0D) {
                    relativeWidth += this.columnSpec[counter];
                } else if (this.columnSpec[counter] == -1.0D && fillWidthRatio != 0.0D) {
                    relativeWidth += fillWidthRatio;
                }
            }

            int temp;
            if (relativeWidth == 0.0D) {
                temp = 0;
            } else {
                temp = (int)((double)scalableWidth / relativeWidth + 0.5D);
            }
            ...
            for(counter = entry.row1; counter <= entry.row2; ++counter) {
                ...
                } else if (this.rowSpec[counter] == -1.0D && fillHeightRatio != 0.0D) {
                    relativeHeight += fillHeightRatio;
                }
            }

            if (relativeHeight == 0.0D) {
                temp = 0;
            } else {
                temp = (int)((double)scalableHeight / relativeHeight + 0.5D);
            }
    ...
    public void addLayoutComponent(Component component, Object constraint) {
        if (constraint instanceof String) {
            ...
            this.list.add(new org.visualdataweb.vowl.protege.TableLayout.Entry(component, (TableLayoutConstraints)constraint));
        } else {
    ...
    protected class Entry extends TableLayoutConstraints {
        ...
        public Entry(Component component, TableLayoutConstraints constraint) {
            ...
            this.singleCell = super.row1 == super.row2 && super.col1 == super.col2;
